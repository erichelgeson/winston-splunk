...
var splunk = exports.splunk = winston.transports.splunk = function (options) {
    ...
    if(options.splunkTransport == 'tcp') {
        this.tcpClient = net.connect(this.splunkPort,this.splunkHost);
    } else {
...
*/
    ...
    if(this.udpClient) {
        ...
        this.tcpClient.write(JSON.stringify(message),function (err, bytes) {
            if (err) {
                callback(err, null);
            } else {
                callback(null, true);
